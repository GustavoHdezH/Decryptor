...
pub fn decrypt_file(path: &Path, output_dir: &Path, passphrase: &str) -> Result<(), String> {
    ...
    if let Some(mut stdin) = gpg_command.stdin.take() {
        if let Err(e) = stdin.write_all(passphrase.as_bytes()) {
            return Err(format!("Error typing passphrase in stdin for {}: {}", path.display(), e));
        }
    }

    let output = gpg_command.wait_with_output()
        .map_err(|e| format!("Error waiting for gpg process for {}: {}", path.display(), e))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(format!("GPG failed for file {}: {}", path.display(), stderr));
    }
