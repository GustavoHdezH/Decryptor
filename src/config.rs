...
pub fn load_config() -> Result<AppConfig, String> {
    ...
    match dotenv() {
        ...
        Err(e) => {
            return Err(format!(
                "Critical error: The .env file could not be loaded. Details: {}",
                e
            ));
        }
    }
    ...
    let input_dir_str = env::var("INPUT_DIR").map_err(|e| {
        format!(
            "Error: The variable 'INPUT_DIR' not found in the .env file. Details: {} ",
            e
        )
    })?;
    let output_dir_str = env::var("OUTPUT_DIR").map_err(|e| {
        format!(
            "Error: The variable 'OUTPUT_DIR' not found in the .env file. Details: {} ",
            e
        )
    })?;
    ...
    let threads = match env::var("SERVER_THREADS") {
        Ok(threads_str) => threads_str
            .parse::<usize>()
            .map_err(|e| format!("Error parsing THREADS: {}", e))?,
        Err(env::VarError::NotPresent) => 0,
        Err(e) => return Err(format!("Error reading THREADS: {}", e)),
    };
    ...
    if !input_dir.is_dir() {
        return Err(format!(
            "Invalid Configuration: The input dir {:?} define in .env does not exists\n",
            input_dir
        ));
    }
    ...
    if !output_dir.is_dir() {
        if let Err(e) = std::fs::create_dir_all(&output_dir) {
            return Err(format!(
                "The output dir '{:?}' define in .env does not exists. Trying create {:?}\n",
                output_dir, e
            ));
        }
    ...
    {
        return Err(format!(
            "The passphrase file '{:?}' defined in .env does not exist.",
            path
        ));
    }
    // Validate the files.
    ...
    {
        return Err(format!(
            "The identity file '{:?}' defined in .env does not exist.",
            path
        ));
    }
